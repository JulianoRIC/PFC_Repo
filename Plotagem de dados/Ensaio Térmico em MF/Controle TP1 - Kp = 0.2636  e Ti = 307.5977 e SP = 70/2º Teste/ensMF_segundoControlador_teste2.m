...
nSam  =  9924; %numero de amostras

fName = '20231124_111540_meas_medsFreio__b.txt';  %valores de tensão e corrente
gName = '20231124_111540_meas_medsWatt__b.txt';   %valores de potencia
hName = '20231124_111540_meas_medsETMA__b.txt';   %temperaturas dos termopares do IF e MA
iName = '20231124_111540_meas_medsETMF__b.txt';   %temperaturas dos termopares do IF e MF
...
axis([0 length(vecIter)/60 19 80])
...
ylabel('TP 1 [ºC]')
legend('SP', 'PV')
axis([118 vecIter(end) 62.77 vecPV(end)+1])
subplot(2,1,2)
plot(vecIter,vecIF)
xlabel('tempo [min]')
ylabel('Corrente [A]')
axis([118 165.4099 0 0.32])
...
plot(minutos,vecSP)
hold off
axis([30 minutos(end) vecPV(1) vecPV(end)+1])
legend('PV','SP');
xlabel('tempo [min]')
ylabel('Temperatura [ºC]')
subplot(2,1,2)
